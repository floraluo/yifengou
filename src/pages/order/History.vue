<template>
  <div>
    <div class="order_list" v-if="show">
      <div class="no_order" v-if="!orderList.length">暂无订单</div>
      <div class="order_item" v-for="(item,index) in orderList" :key="index" @click="gotoHistoryDetail(item)">
        <div>{{item.name}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      gotoHistoryDetail(item) {
        this.$router.push({path:"/order/history/detail", query:{activity: item.activity, name: item.name}});
      }
    },
    mounted() {
      this.initView();
